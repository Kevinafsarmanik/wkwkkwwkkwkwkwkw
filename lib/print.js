...
module.exports = async function (m, conn = { user: {} }) {
  ...
  console.log(`${chalk.redBright('%s')} ${chalk.black(chalk.bgGreen('%s'))} ${chalk.magenta('%s [%s %sB]')}
${chalk.green('%s')} ${chalk.yellow('%s%s')} ${chalk.blueBright('de')} ${chalk.green('%s')} ${chalk.black(chalk.bgYellow('%s'))}`.trim(),
    me + ' ~' + conn.user.name,
