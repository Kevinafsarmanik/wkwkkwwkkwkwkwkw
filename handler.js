...
module.exports = {
    async handler(chatUpdate) {
      ...
      //  global.bg = await (await fetch(img)).buffer()
        global.time = require('moment-timezone').tz('America/Santiago').format('HH:mm:ss')
        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!('job' in user)) user.job = 'Desempleo'
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.sendButton(m.chat, `Su límite ha aumentado, compre a través de *${usedPrefix}buy*`, wm, 'Buy', '.buy', m)
                        // this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`, m)
                        continue // Limit habis
                    }
                    if (plugin.level > _user.level) {
                        this.sendButton(m.chat, `nivel requerido ${plugin.level} para usar este comando. Tu nivel es ${_user.level}`, wm, `Levelup`, `.levelup`, m)
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            await this.send3TemplateButtonImg(id, action === 'add' ? wel : lea, text, wm, action === 'add' ? 'Bienvenido' : 'Nos vemos', action === 'add' ? '.intro' : 'FokusID')
                        }
                    }
                }
                break
    ...
    async groupsUpdate(groupsUpdate, fromMe, m) {
        ...
        for (let groupUpdate of groupsUpdate) {
            ...
            if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '```El ícono ha sido cambiado a```').replace('@icon', groupUpdate.icon)
            if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || '```El enlace del grupo se ha cambiado a```\n@revoke').replace('@revoke', groupUpdate.revoke)
            if (groupUpdate.announce == true) text = (chats.sAnnounceOn || this.sAnnounceOn || conn.sAnnounceOn || '```¡El grupo ha sido cerrado!')
            if (groupUpdate.announce == false) text = (chats.sAnnounceOff || this.sAnnounceOff || conn.sAnnounceOff || '```El grupo ha estado abierto!')
            if (groupUpdate.restrict == true) text = (chats.sRestrictOn || this.sRestrictOn || conn.sRestrictOn || '```Grupo ha sido configurado para todos los participantes!')
            if (groupUpdate.restrict == false) text = (chats.sRestrictOff || this.sRestrictOff || conn.sRestrictOff || '```Grupo ha sido configurado para solo admins!')
            //console.log('=============\n\ngroupsUpdate \n\n============\n\n' + await groupUpdate)
            if (!text) continue
            await this.sendButton(id, text, wm, 'Desactivar funciones"', `.off detect`, global.ftroli, { contextInfo: { mentionedJid: this.parseMention(text) }, mentions: await this.parseMention(text) })
...
*.enable delete*
`.trim(), wm, 'Desactivar esta característica', '.enable delete', msg, {
...
global.dfail = async (type, m, conn) => {
    let msg = {
        rowner: `¡Este comando solo puede ser utilizado por un _*Rowner*_`,
        owner: `Este comando solo puede ser utilizado por mi _*Owner!*_`,
        mods: `Este comando solo puede ser utilizado por *Moderadores*`,
        premium: 'Este comando es solo para usuarios _*Premium*_ !',
        ...
        unreg: 'Regístrese para usar esta función escribiendo:\n\n*#daftar nombre.edad*\n\nEjemplo: *#daftar de juan.21*',
        nsfw: `NSFW no está activo, comuníquese con un moderador para activar esta función!`,
        rpg: `RPG no está activo, comuníquese con un moderador para activar esta función!`,
