...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `uhm... ¿dónde está la URL?\n\nEjemplo:\n${usedPrefix + command} https://t.me/addstickers/paquete`
    if (!args[0].match(/(https:\/\/t.me\/addstickers\/)/gi)) throw `URL errónea`
    try {
...
*Finalización estimada:* ${json.result.stickers.length * 1} segundos`.trim())
