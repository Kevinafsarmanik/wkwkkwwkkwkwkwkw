let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0] || isNaN(args[0])) throw `¡Ingrese un número que represente la cantidad de días!\n*Ejemplo : ${usedPrefix + command} 30*`
    ...
    conn.reply(m.chat, `Establecer con éxito el día de vencimiento para este grupo a ${args[0]} hari.\n\nCuenta regresiva : ${msToDate(global.db.data.chats[who].expired - now)}`, m)
}
handler.help = ['expired <dias>']
handler.tags = ['owner']
