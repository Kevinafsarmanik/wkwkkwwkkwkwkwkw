...
let handler = async(m, { conn, args, isPrems, isOwner }) => {
    ...
    conn.reply(isLimit ? `Tamaño del archivo: ${filesizeF}\nlimite ${limit} MB, descárgalo tú mismo: ${dl_link}` : global.wait, m)
    let _thumb = {}
    try { _thumb = { thumbnail: await (await fetch(thumb)).buffer() } } catch (e) {}
    m.reply(wait)
    if (!isLimit) await conn.sendButtonVid(m.chat, dl_link, `*Tituló:* ${title}\n*Peso:* ${filesizeF}`.trim(), wm, 'menu', '.?', m)
