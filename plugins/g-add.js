...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
	...
	if (new Date - chat.add < timeout) throw `_¡Este comando todavía está en enfriamiento! _\nPor favor, espere *${conn.msToDate(time - new Date())}*!`
    ...
    for (let jid of response) {
        ...
        if (__user) {
            conn.sendButton(m.chat, `_Agregado exitosamente @${jid.split`@`[0]}_`, wm, 'menu', usedPrefix + 'menu', m)
        }
        if (!__user) {
            conn.sendButton(m.chat, `_No se pudo agregar @${jid.split`@`[0]}!_\nTal vez el usuario acaba de salir de este grupo por lo que tiene que entrar a través de *${usedPrefix}link*`, wm, 'link', usedPrefix + 'link', m)
            /*
