...
handler.before = async function (m) {
    ...
    if (m.isGroup && global.db.data.chats[m.chat].expired != 0) {
        if (new Date() * 1 >= global.db.data.chats[m.chat].expired) {
            this.reply(m.chat, `tiempo *${this.user.name}* para dejar el grupo, ¿quieres que este bot permanezca en tu grupo? alquilar al número de abajo oh solo pide agregarlo y seras añadido a una lista de espera`, null).then(() => {
